...
namespace TriInspector.Drawers
{
    public class MinMaxSliderAttributeDrawer : TriAttributeDrawer<MinMaxSliderAttribute>
    {
        ...
        public override void OnGUI(Rect position, TriProperty property, TriElement next)
        {
            ...
            {
                var value = (Vector2Int) property.Value;
                xValue = value.x;
                yValue = value.y;
            }
            else
            {
                var value = (Vector2) property.Value;
                xValue = value.x;
                yValue = value.y;
            }
    ...
    internal static class MinMaxSliderAttributeHelpers
    {
        ...
        {
            internal SliderResolvers(ref HashSet<string> errors,
                TriPropertyDefinition propertyDefinition,
                MinMaxSliderAttribute attribute)
                : base(ref errors, propertyDefinition,
                    attribute.MinMemberName, attribute.MaxMemberName, attribute.MinMaxMemberName)
            {
            }
        }
        ...
        public static SliderResolvers Initialize(MinMaxSliderAttribute attribute,
            TriPropertyDefinition propertyDefinition,
            out TriExtensionInitializationResult errorResult)
        {
            var errors = new HashSet<string>();

            if (propertyDefinition.FieldType != typeof(Vector2) && propertyDefinition.FieldType != typeof(Vector2Int))
            {
                errors.Add("[MinMaxSlider] attribute can only be used on Vector2 or Vector2Int fields.");
            }
        ...
        public static (double min, double max) GetLimits(TriProperty property,
            MinMaxSliderAttribute attribute,
        ...
        public static void SetValue(TriProperty property, float x, float y)
        {
            ...
            {
                property.SetValue(new Vector2Int(Mathf.RoundToInt(x), Mathf.RoundToInt(y)));
            }
            else
            {
